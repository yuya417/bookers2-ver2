...
describe '[STEP4] いいねとコメントのテスト' do
  ...
  let!(:user_book_comment) { create(:book_comment, book:, user:) }
  let!(:user_other_favorite) { create(:favorite, book: other_book, user:) }  # 削除テスト用に、他人の投稿に対して自分がいいねを作成
  let!(:other_user_book_comment) { create(:book_comment, book:, user: other_user) }
    ...
    context '表示の確認' do
      ...
      it '他人が投稿したコメントの削除ボタンは表示されない' do
        expect(page).not_to have_link 'Destroy',
                                      href: book_book_comment_path(book_id: book.id, id: other_user_book_comment.id)
      end
    ...
    context 'コメント追加機能のテスト' do
      ...
      it 'リダイレクト先で新しいコメントが表示されている' do
        ...
        expect(page).to have_link 'Destroy',
                                  href: book_book_comment_path(book_id: book.id, id: user.book_comments.last.id)
      end
    end
